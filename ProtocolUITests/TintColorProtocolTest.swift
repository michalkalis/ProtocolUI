...
class TintColorProtocolTest: XCTestCase {
    ...
    func performTestWithClass(_ classType : UIView.Type, shouldTestIBDesignable: Bool = false) {
        ...
        if let pageControl = testView as? UIPageControl {
            ...
            XCTAssertEqual(pageControl.pageIndicatorTintColor, type(of: self).testValue)
            
        } else {
            
            XCTAssertEqual(testView.tintColor, type(of: self).testValue)
        }
    }
    ...
    func testUIBarButtonItem() {
        ...
        XCTAssertEqual(test1.tintColor, type(of: self).testValue)
        
        
        let test2 = TestView()
        test2.prepareForInterfaceBuilder()
        
        XCTAssertEqual(test2.tintColor, type(of: self).testValue)
    }
